...
class Model(nn.Module):
    ...
    def __init__(self, configs, individual=False):
        ...
        print('Initializing DLinear Model...')
...
#         self.individual = individual
    ...
    def encoder(self, x, mode):
        ...
        # pred_from_retrieval = self.retrieval_dict[mode][:, index] # G, B, P, C。 G是周期数。
        ...
        pred = x_pred_from_x   # 单纯用x的线性预测，不结合检索结果。
    ...
    def classification(self, x_enc, mode):
        # Encoder
        enc_out = self.encoder(x_enc, mode)
        # Output
    ...
    def forecast(self, x_enc, mode):
        # Encoder
        return self.encoder(x_enc, mode)

    def anomaly_detection(self, x_enc, mode):
        # Encoder
        return self.encoder(x_enc, mode)

    def forward(self, x_enc, batch_x_mark, dec_inp, batch_y_mark, mode='train'):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            dec_out = self.forecast(x_enc, mode)
            return dec_out[:, -self.pred_len:, :]  # [B, L, D]
        if self.task_name == 'classification':
            dec_out = self.classification(x_enc, mode)
            return dec_out  # [B, N]
        if self.task_name == 'anomaly_detection':
            dec_out = self.anomaly_detection(x_enc, mode)
