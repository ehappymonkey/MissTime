...
from layers.onlineRetrieval import Retrieval
# from layers.latentRetrieval import latentRetrieval
from layers.latentRetrieval_obs import latentRetrieval
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if configs.rag_type == 'latent_rag':
            self.rt = latentRetrieval(
                ...
                temperature=0.1,
                device=self.device,
                retriever=configs.retrieve_encoder, # default 'TimerXL'
            )
    ...
    def prepare_dataset(self, train_data, task_name=None):
        if self.args.rag_type == 'feature_rag' or self.args.rag_type == 'latent_rag':
            self.rt.prepare_dataset(train_data, task_name, self.args.data) # rt里得到train_data_all_mg。
