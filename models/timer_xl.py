...
class Model(nn.Module):
    ...
    def get_embedding(self, x, x_mark=None, y_mark=None):
        if self.use_norm:
            means = x.mean(1, keepdim=True).detach()
            x = x - means
            stdev = torch.sqrt(
        ...
        embed_out = embed_out.reshape(B, C * N, -1)
        embed_out, attns = self.blocks(embed_out, n_vars=C, n_tokens=N)
        embedding_tensor = embed_out.reshape(B, C, N, -1) # [B, C, N, D]
        last_patch_embedding = embedding_tensor[:, :, -1, :] # [B, C, D]
        return last_patch_embedding
