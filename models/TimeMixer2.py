...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if getattr(configs, 'rag_type', 'no_rag') == 'latent_rag':
            self.rt = missRetrieval(
                configs, 
                topk=getattr(configs, 'topm', 5),
                temperature=0.1,
                device=self.device,
                retriever=getattr(configs, 'retrieve_encoder', None), 
            )
        # ### ----------------------- ###
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, batch_mask, x_full, mode):
        ...
        if mode != 'train' and hasattr(self, 'rt'):
            if getattr(self.args, 'rag_type', 'no_rag') == 'latent_rag':
                ...
                if getattr(self.args, 'rag_strategy', 'channels') == 'channels':
                    x_recon = self.rt.retrieve_recon(x_enc, x_mark_enc, _batch_mask) 
                elif self.args.rag_strategy == 'whole':
                    x_recon = self.rt.retrieve_recon_whole(x_enc, x_mark_enc, _batch_mask)
                x_enc = torch.where(_batch_mask.unsqueeze(1).bool().to(x_enc.device), x_enc, x_recon)
    ...
    def classification(self, x_enc, x_mark_enc, batch_mask, x_full, mode):
        ...
        if mode != 'train' and hasattr(self, 'rt'):
            ...
            elif getattr(self.args, 'rag_type', 'no_rag') in['feature_rag', 'latent_rag']:
                x_recon = self.rt.retrieve_recon(x_enc, observed_mask=batch_mask)
            else:
                x_recon = x_enc
    ...
    def anomaly_detection(self, x_enc, batch_mask, x_full, mode):
        ...
        if mode != 'train' and hasattr(self, 'rt'):
            if getattr(self.args, 'rag_type', 'no_rag') == 'latent_rag':
                _batch_mask = batch_mask.unsqueeze(0).expand(x_enc.shape[0], -1)
                x_recon = self.rt.retrieve_recon(x_enc, None, _batch_mask) 
                x_enc = torch.where(_batch_mask.unsqueeze(1).bool().to(x_enc.device), x_enc, x_recon)
            elif self.args.rag_type == 'no_rag':
                x_recon = x_enc   
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, batch_mask=None, batch_x_full=None, mode='train', mask=None):
        ...
        if self.task_name == 'imputation':
            dec_out = self.imputation(x_enc, x_mark_enc, batch_mask, batch_x_full, mode, mask)
