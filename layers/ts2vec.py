...
import numpy as np
from layers.tsencoder import TSEncoder
from layers.utils_ts2vec import take_per_row, split_with_nan, centerize_vary_length_series, torch_pad_nan
import math
...
class TS2Vec:
    ...
    def encode(self, data, mask=None, encoding_window=None, causal=False, 
        ...
        Args:
            data (Union[numpy.ndarray, torch.Tensor]): Shape (n_instance, n_timestamps, n_features).
                - If torch.Tensor: should be float32, missing data as NaN, device can be CPU/GPU.
                - If numpy.ndarray: missing data as NaN.
        '''
        ...
        if torch.is_tensor(data):
            ...
            device = self.device
            n_samples, ts_l, _ = data.shape
            tensor_data = torch.from_numpy(data).to(torch.float32).to(device)
        ...
        with torch.no_grad():
            output = []
            for batch in loader:
                x = batch[0]  # Already on target device
            ...
            final_output = torch.cat(output, dim=0)
    ...
    def encode_cpu(self, data, mask=None, encoding_window=None, causal=False, sliding_length=None, sliding_padding=0, batch_size=None):
        ''' Compute representations using the model.
